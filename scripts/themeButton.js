/**
 * Fancy animated theme toggle custom element.
 * Adapted from user-provided implementation.
 */
...
function setupThemeButton(root, initialTheme, changeTheme) {
    const $ = (selector) => {
    ...
    components.addEventListener("click", handleToggle);
    ...
    mainButton.addEventListener("mousemove", handleMouseMove);
    mainButton.addEventListener("mouseout", handleMouseOut);
    ...
    const getRandomDirection = () => (Math.random() > 0.5 ? "2em" : "-2em");
    ...
    return {
        ...
        destroy() {
            components.removeEventListener("click", handleToggle);
            mainButton.removeEventListener("mousemove", handleMouseMove);
            mainButton.removeEventListener("mouseout", handleMouseOut);
